...
class SudoKu():
    ...
    def _judge_value_is_legal(self,row,column,value):
        ...
        # value是否存在这一行数据中
        if row in self.every_row_data.keys(): 
            if value in self.every_row_data[row]:
                return False

        # value是否存在这一列数据中
        if column in self.every_column_data.keys(): 
            if value in self.every_column_data[column]:
                return False

        # value是否存在这个3*3的宫内
        key = self._get_three_to_three_key(row,column)
        if key in self.every_three_to_three_data.keys(): 
...
#        print(f"run backtracking... value is {previous_value},vacant position is {previous_vacant_position}")
    ...
    def show_sudo_ku(self):
        ...
        '''
        print("Result:")
        for index,row in enumerate(self.sudo_ku):
            if index % 3 == 0:
                print("+-----+-----+-----+")
...
if __name__ == '__main__':
    ...
    for index,number in enumerate(sudoku_number):
        index1 = index//9
        index2 = index%9
        if number == "0":
            sudo_ku_data[index1][index2] = ""
        else:
            sudo_ku_data[index1][index2] = int(number)

    print("Your input:")
    for i in range(9):
        if i % 3 == 0:
            print("+-----+-----+-----+")
        print("|" + sudoku_number[i*9 + 0].replace('0',' ') + " " + sudoku_number[i*9 + 1].replace('0',' ') + " " + sudoku_number[i*9 + 2].replace('0',' ') + "|" + sudoku_number[i*9 + 3].replace('0',' ') + " " + sudoku_number[i*9 + 4].replace('0',' ') + " " + sudoku_number[i*9 + 5].replace('0',' ') + "|" + sudoku_number[i*9 + 6].replace('0',' ') + " " + sudoku_number[i*9 + 7].replace('0',' ') + " " + sudoku_number[i*9 + 8].replace('0',' ') + "|")
    print("+-----+-----+-----+")
...
#    print(judge_sudo_ku_is_legal(sudo_ku))
    judge_sudo_ku_is_legal(sudo_ku)
